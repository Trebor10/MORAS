...
Proof.
  ...
  intros H. simpl. rewrite H. rewrite P_NotL. rewrite P_AddL. reflexivity.
Qed.
...
Proof.
  ...
  assert (P : forall (x : list B), AndL x (repeat I (length x)) = x ).
  {
    intros. induction x0.
    - simpl. reflexivity.
    - simpl. rewrite IHx0. rewrite P_And. reflexivity.
  }
  intros. simpl. rewrite P_NotL. rewrite <- H. rewrite P. reflexivity.  
Qed.
...
Proof.
  ...
  assert (P : forall (x : list B), AndL (repeat I (length x)) x = x ).
  {
    intros. induction x0.
    - simpl. reflexivity.
...
Proof.
  ...
  intros. simpl. rewrite H. rewrite P_NotL. rewrite P. reflexivity.  
Qed.
...
Proof.
  ...
  assert (P_rev_2 : forall (b : B) (n : nat), rev (repeat b n) = repeat b n).
  {
    intros. induction n0.
    - reflexivity.
    - simpl. rewrite IHn0. rewrite P_rev_1. reflexivity.
  }
  assert (P_rep_S: forall (n : nat), I :: repeat I n = repeat I (S n)).
  {
    simpl. intros. reflexivity.
  }
  ...
  assert (P_AddL: AddL (repeat I n) (repeat I n) = repeat I (pred n) ++ [O]).
  {
    ...
    - simpl. contradiction.
    - simpl. rewrite P_rep_S. unfold AddL. rewrite P_rev_2. simpl. rewrite P_AddLr. rewrite P_rev_2. reflexivity.   
  }
  assert (P_NotL_2: forall (n : nat), NotL (repeat I n ++ [O]) = repeat O n ++ [I]).
  {
  ...
  induction n.
  - contradiction.
  - simpl. rewrite P_NotL. rewrite P_rep_S. rewrite P_AddL. simpl. rewrite P_NotL_2. reflexivity. 
Qed.
